...
const TripRecommendations: React.FC = () => {
    ...
    // ===== normalize/lookup for landmarks by Name =====
    ...
    const getLandmarkThumbUrl = (
        ...
        // 2) partial contains: 'paris, france' <-> 'paris'
        for (const c of candidates) {
    ...
    // สุ่มหยิบ N ตัวแบบเร็ว (partial Fisher–Yates)
    ...
    /* =========================
     * UI: One review row/card (internal)
     * =======================*/
    const ReviewCard: React.FC<{
        ...
        const title =
            (trip as any)?.Name?.toString?.() ||
            (condition as any)?.Landmark?.toString?.() ||
            "-";
        ...
        const handleClick = () => {
            if (trip && (trip as any).ID) {
                ...
                console.log("Saved recommendTripID:", tripId);
    ...
    const load = useCallback(async () => {
        ...
        setLoading(true);
        try {
            // 0) Landmarks → map<Name, ThumbnailURL>
            ...
            // 6) random 5 fast
            const randomFive = pickRandomN(highRated, 4);
