...
const pickLatLon = (obj: any): { lat: number; lon: number } | null => {
  ...
  const wktTry = parseWktPoint(obj.location || obj.Location);
  if (wktTry) return wktTry;

  const latTop = obj.lat ?? obj.latitude ?? obj.Lat ?? obj.Latitude ?? obj.latitudes ?? null;
  const lonTop = obj.lon ?? obj.lng ?? obj.longitude ?? obj.Lon ?? obj.Longitude ?? obj.long ?? null;
...
const hideAllUi = (map: any) => {
  try {
    ...
    const tryHide = (x: any) => {
      if (!x) return;
      if (typeof x.visible === "function") { try { x.visible(false); } catch {} }
      if (typeof x.display === "function") { try { x.display(false); } catch {} }
      if (typeof x.enable === "function")  { try { x.enable(false); }  catch {} }
    };
...
const MapRoute: React.FC = () => {
  ...
  useEffect(() => {
    ...
    refreshTripId();
    const onStorage = (e: StorageEvent) => { if (e.key === "TripID") refreshTripId(); };
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const rows = (all || []).filter((r) => Number(r.TripID) === Number(tripId) && r.ToCode);
        ...
        for (const [d, arr] of Array.from(dayMap.entries()).sort((a, b) => a[0] - b[0])) {
          ...
          for (const sp of arr) {
            ...
            if (cache.has(code)) { points.push(cache.get(code)!); continue; }
            ...
            const p: PlacePoint = { code, kind, idNum, day: d, name: nameOf(fetched), lat: ll.lat, lon: ll.lon };
            cache.set(code, p);
            points.push(p);
          }
        ...
        if (!mounted) return;
        const sortedDays = Object.keys(resultByDay).map(Number).sort((a, b) => a - b);
  ...
  const safeFitToPoints = (map: any, points: PlacePoint[]) => {
    ...
    try {
      if (typeof map.bound === "function") {
        map.bound(points.map(p => ({ lon: p.lon, lat: p.lat })), { animate: true, padding: 56 });
        return;
      }
    } catch {}
    ...
    <div className="map-route">
      <div className="map-toolbar">
        <Space align="center" size={6}>
          ...
          />
          <Tag className="day-pill">
            {dayFilter == null ? "วันที่ —" : `วันที่ ${dayFilter}`}
          </Tag>
          <Button
            size="small"
            shape="circle"
      ...
      {error && <div className="map-error"><small>⚠ {error}</small></div>}
